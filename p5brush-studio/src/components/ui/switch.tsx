...
const Switch = React.forwardRef<HTMLButtonElement, SwitchProps>(({ className, onCheckedChange, ...props }, ref) => (
  <BaseSwitch.Root
    ...
    className={cn(
      'group inline-flex h-[22px] w-[38px] shrink-0 cursor-pointer items-center rounded-full p-[2px] outline-none transition-colors duration-150',
    ...
    {...props}
  >
    <BaseSwitch.Thumb className="block h-[18px] w-[18px] rounded-full bg-white shadow-[0_1px_2px_rgba(28,24,18,0.3)] transition-transform duration-150 ease-out data-[checked]:translate-x-4" />
  </BaseSwitch.Root>
));
